...
namespace SharpBrowser {
	...
	internal partial class MainForm : Form {
		...
		public static string ServerURLorIP = "";


		public static string Branding = "WebBrowser_" + Version;
		...
		public static string HomepageURL = ServerURLorIP + "/home";
		public static string NewTabURL = ServerURLorIP + "/home";
		...
		public static string SearchURL = "https://www.pornhub.com/video/search?search=";
		public static string WebMappingString = "";
		public static string DNSVersion = "";
		public static string DNSServerURL = ServerURLorIP + "/dns";
		...
		private void InitDNSConfigServer() {
			Log.WriteLog("向服务器请求DNS数据...");
			string response = SharpBrowser.HttpUtils.GetHttpResponse(DNSServerURL);
			if (!string.IsNullOrEmpty(response))
			{
				try
				{
					ApiDNSList dnsList = JsonConvert.DeserializeObject<ApiDNSList>(response);
					DNSVersion = dnsList.v;
					foreach (KeyValuePair<string, string> oneDns in dnsList.dns)
					{
						var value = "MAP " + oneDns.Key + " " + oneDns.Value + ",";
						WebMappingString += value;
					}
					if (!String.IsNullOrEmpty(WebMappingString))
					{
						WebMappingString = WebMappingString.Substring(0, WebMappingString.Length - 1);
					}
					Log.WriteLog(String.Format("获取DNS数据映射 {0} 条", dnsList.dns.Count));
				}
                catch (Exception)
                {
					Log.WriteLog("请求DNS数据格式不正确...");
				}
            }
            else
            {
				Log.WriteLog("请求DNS数据失败...");
			}
		}


		public void InitServerConfig() {
			Log.WriteLog("初始化配置加载...");
			foreach (string line in System.IO.File.ReadLines("serverip.txt"))
			{
				if (line.StartsWith("#"))
				{
					continue;
				...
				else
				{
					...
					{
						ServerURLorIP = "http://" + ServerURLorIP;
					}
					Log.WriteLog("服务器地址为: " + ServerURLorIP);
					DNSServerURL = ServerURLorIP + "/dns";
					HomepageURL = ServerURLorIP + "/home";
					NewTabURL = ServerURLorIP + "/home";
				}
			}
			Log.WriteLog("配置加载完成...");
		}
		public void InitLoadConfig() {
			// WebMappingString = "MAP *.xvideos2.com 104.18.3.88,MAP *.pornhub.com 66.254.114.41";
			Log.WriteLog("初始化配置加载...");
			WebMappingString = "";

			foreach (string line in System.IO.File.ReadLines("myHosts.txt"))
			{
				if (line.StartsWith("#"))
				{
					continue;
		...
		private void InitBrowser() {
			...
			Log.WriteLog("初始化Cef3...");
			CefSettings settings = new CefSettings();
			if (!String.IsNullOrEmpty(WebMappingString))
			{
				settings.CefCommandLineArgs.Add("host-resolver-rules", WebMappingString);
				Log.WriteLog("设置映射关系: " + WebMappingString);
			}
